...
export const getSitesBySearchTerm = function (
	...
	if ( type ) {
		for ( const siteId in sites ) {
			if ( sites[ siteId ][ 'templatiq-sites-type' ] === type ) {
	...
	for ( const siteId in sites ) {
		 ...
		 */
		if ( site[ 'templatiq-sites-tag' ] ) {
			for ( const tagId in site[ 'templatiq-sites-tag' ] ) {
				const tag = site[ 'templatiq-sites-tag' ][ tagId ].replace(
					'-',
					' '
				);
