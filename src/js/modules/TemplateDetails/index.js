...
export default function TemplateDetailsModule(props) {
	...
	const fetchData = async () => {
		try {
			...
			const singleTemplate = templateData && templateData.find((template) => template.slug === templateSlug);
			...
			const storeUpdate = subscribe(() => {
				...
				const updatedSingleTemplate = updatedTemplateData && updatedTemplateData.find((template) => template.slug === templateSlug);
