...
export default function CreatePopupAction( props ) {
	...
	return (
		<Link
			to={ `${ url }` || '#' }
			className="template-market-createPopup__action"
			onClick={ ( e ) =>
				step ? handleCreateFormSteps( e, step ) : null
			}
		>
			<div className="template-market-createPopup__action-icon">
				<ReactSVG src={ icon } />
			</div>
			{ text && (
				<span className="template-market-createPopup__action-text">
					{ text }
				</span>
			) }
		</Link>
