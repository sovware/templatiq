...
namespace TemplateMarket\Elements\SingleLocation;

use TemplateMarket\Elements\AllListings\Styles as AllListingStyles;
...
class SingleLocation extends Widget_Base {
	...
	public function get_title() {
		return __( 'Single Location', 'template-market' );
	}
	...
	protected function register_contents(): void{
		...
		$this->add_control(
			'show_pagination',
			[
				'label'     => __( 'Show Pagination?', 'template-market' ),
				'type'      => Controls_Manager::SWITCHER,
				'label_on'  => esc_html__( 'Show', 'template-market' ),
				'label_off' => esc_html__( 'Hide', 'template-market' ),
				'default'   => 'no',
	...
	protected function register_styles(): void {
		...
		$this->register_container_style_controls( __( 'Directory Type: Container', 'template-market' ), 'single_location_directory_type_container', '.directorist-type-nav__list' );
		...
		$this->register_container_style_controls( __( 'Listing Header: Container', 'template-market' ), 'single_location_header_container', '.directorist-listings-header' );

		$this->register_filters_button( ['header' => 'yes', 'advanced_filter' => 'yes'] );

		$this->register_text_controls( __( 'Listing Header: Listings Found', 'template-market' ), 'single_location_listings_found', '.directorist-listings-header__left .directorist-header-found-title' );

		$this->register_view_as_sort_by();

		$this->register_container_style_controls( __( 'Listing: Card Container', 'template-market' ), 'single_location_listing_card_container', '.directorist-listing-card' );

		$this->register_listing_card_info();
		$this->register_listing_footer();

		$this->register_container_style_controls( __( 'Pagination Container', 'template-market' ), 'single_location_pagination_container', '.directorist-pagination', ['show_pagination' => 'yes'] );
	...
	protected function render(): void{
		...
		$atts = [
			...
			'listings_per_page'     => $settings['number_loc'],
		...
		/**
		 * Filters the Elementor Single Location atts to modify or extend it
		 *
		 * @since 1.0.0
		 *
		 ...
		 */
		$atts = apply_filters( 'directorist_location_elementor_widget_atts', $atts, $settings );

		Helper::run_shortcode( 'directorist_location', $atts );
	}
}
