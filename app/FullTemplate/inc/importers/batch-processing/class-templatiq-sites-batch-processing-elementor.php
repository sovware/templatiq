 ...
 * @since 1.0.0
 */
class Templatiq_Sites_Batch_Processing_Elementor extends Source_Local {
	...
	public function import() {
		...
		\Templatiq_Sites_Importer_Log::add( '---- Processing WordPress Posts / Pages - for Elementor ----' );
		$post_types = \Templatiq_Sites_Batch_Processing::get_post_types_supporting( 'elementor' );
	...
	public function import_single_post( $post_id = 0 ) {
		...
		\Templatiq_Sites_Importer_Log::add( '---- Processing WordPress Page - for Elementor ---- "' . $post_id . '"' );

		if ( ! empty( $post_id ) ) {

			$data = get_post_meta( $post_id, '_elementor_data', true );

			\Templatiq_Sites_Importer_Log::add( wp_json_encode( $data ) );

			if ( ! empty( $data ) ) {

				// Update WP form IDs.
				$ids_mapping = get_option( 'templatiq_sites_wpforms_ids_mapping', array() );
				\Templatiq_Sites_Importer_Log::add( wp_json_encode( $ids_mapping ) );
				if ( $ids_mapping ) {
				...
				update_metadata( 'post', $post_id, '_templatiq_sites_hotlink_imported', true );
			...
			$clean_post_excerpt = apply_filters( 'templatiq_sites_pre_process_post_empty_excerpt', true );
