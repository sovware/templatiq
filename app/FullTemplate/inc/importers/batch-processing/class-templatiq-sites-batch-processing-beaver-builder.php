...
if ( ! class_exists( 'Templatiq_Sites_Batch_Processing_Beaver_Builder' ) ) :

	/**
	 * Templatiq_Sites_Batch_Processing_Beaver_Builder
	 *
	 * @since 1.0.14
	 */
	class Templatiq_Sites_Batch_Processing_Beaver_Builder {
		...
		public function import() {
			...
			Templatiq_Sites_Importer_Log::add( '---- Processing WordPress Posts / Pages - for Beaver Builder ----' );
			...
			$post_ids = Templatiq_Sites_Batch_Processing::get_pages( $post_types );
		...
		public function import_single_post( $post_id = 0 ) {
			...
			$imported_from_demo_site = get_post_meta( $post_id, '_templatiq_sites_enable_for_batch', true );
			...
			Templatiq_Sites_Importer_Log::add( '---- Processing WordPress Page - for Beaver Builder ---- "' . $post_id . '"' );
			...
			if ( ! empty( $post_id ) ) {
				...
				templatiq_sites_empty_post_excerpt( $post_id );
		...
		public static function update_module( $settings ) {
			...
			if ( 'uabb-wp-forms-styler' === $settings->type ) {
				templatiq_sites_error_log( '--------WP Form Styler ID replacement start-------' );
				$ids_mapping = get_option( 'templatiq_sites_wpforms_ids_mapping', array() );
				if ( $ids_mapping ) {
					// Update WP form IDs.
					foreach ( $ids_mapping as $old_id => $new_id ) {
						if ( isset( $settings->wp_form_id ) && $old_id === $settings->wp_form_id ) {
							templatiq_sites_error_log( '--------WP Form Styler ID ' . $old_id . ' replaced to ' . $new_id . '-------' );
							$settings->wp_form_id = $new_id;
						}
					}
				}
				templatiq_sites_error_log( '--------WP Form Styler ID replacement done-------' );
			}
		...
		private static function get_wpforms_mapping( $content = '' ) {
			$ids_mapping = get_option( 'templatiq_sites_wpforms_ids_mapping', array() );
			templatiq_sites_error_log( wp_json_encode( $ids_mapping ) );
			if ( $ids_mapping ) {
				// Update WP form IDs.
				foreach ( $ids_mapping as $old_id => $new_id ) {
	 ...
	 */
	Templatiq_Sites_Batch_Processing_Beaver_Builder::get_instance();
