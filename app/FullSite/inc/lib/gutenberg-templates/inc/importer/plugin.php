...
class Plugin {
	...
	private function __construct() {
		...
		if ( ( isset( $_GET['action'] ) && 'edit' === $_GET['action'] ) || ( isset( $_SERVER['REQUEST_URI'] ) && strpos( esc_url_raw( $_SERVER['REQUEST_URI'] ), 'post-new.php' ) !== false ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function init() {
		...
		if ( isset( $_GET['ast_action'] ) && 'auth' === $_GET['ast_action'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$token = isset( $_GET['credit_token'] ) ? sanitize_text_field( wp_unslash( $_GET['credit_token'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$business_details = get_option( 'ast-templates-business-details', false );
			if ( ! $business_details ) {
				$business_details = array();
			}

			$business_details['token'] = Helper::encrypt( $token );
		...
		// Add token if user already authorised from ZipAI.
		if ( empty( Importer_Helper::get_business_details( 'token' ) ) ) {
			$spec_ai_auth_token = Helper::get_setting( 'auth_token' );

			if ( ! empty( $spec_ai_auth_token ) ) {
				$business_details['token'] = $spec_ai_auth_token;
				update_option( 'ast-templates-business-details', $business_details );
			}
		}

		// Remove token if user revoke access from ZipAI.
		if ( empty( Helper::get_setting( 'auth_token' ) ) && defined( 'ZIP_AI_DIR' ) ) {
	...
	public function import_block() {
		...
		$is_templatiq_theme = class_exists( 'Templatiq_Global_Palette' );
		...
		if ( 'block' === $block_type ) {
			...
			$color_palettes = ! $is_templatiq_theme ? $this->get_block_palette_colors()[ $style ]['colors'] : $color_palettes;
		} else {
			$mapping_palette = array(
			...
			$color_palettes = ! $is_templatiq_theme ? $this->get_page_palette_colors()[ $style ]['colors'] : $color_palettes;
		}

		if ( ! $is_templatiq_theme ) {
			for ( $i = 0; $i < 9; $i++ ) {
	...
	public function template_assets() {
		...
		$templatiq_theme_css = apply_filters( 'templatiq_dynamic_theme_css', '' );
		$templatiq_theme_css = str_replace( ':root', '', $templatiq_theme_css );
		$templatiq_theme_css = str_replace( 'body', '', $templatiq_theme_css );
		...
		$ast_footer = '';
		$static_css_path = '';
		if ( defined( 'TEMPLATIQ_THEME_VERSION' ) ) {
			$templatiq_customizer_css = ( class_exists( 'Templatiq_Dynamic_CSS' ) ) ? \Templatiq_Dynamic_CSS::return_output( '' ) : '';
			ob_start();
			$ast_header = templatiq_header_markup();
			$ast_header = ob_get_clean();

			ob_start();
			$ast_footer = templatiq_footer_markup();
			$ast_footer = ob_get_clean();
			$static_css_path = TEMPLATIQ_THEME_DIR . 'assets/css/minified/main.min.css';
		} else {
			$templatiq_customizer_css = get_option( 'ast-block-templates-customizer-css', '' );
			if ( empty( $templatiq_customizer_css ) ) {
				Sync_Library::instance()->maybe_get_server_templatiq_customizer_css();
				$templatiq_customizer_css = get_option( 'ast-block-templates-customizer-css', '' );
			}
		}
		...
		$spec_ai_auth_url = add_query_arg( $request_params, 'https://app.zipwp.com/auth/' );
		...
		wp_localize_script(
			...
			apply_filters(
				...
				array(
					...
					'templatiq_sites_pro_status'  => $this->get_plugin_status( 'astra-pro-sites/astra-pro-sites.php' ),
					'wpforms_status'          => $this->get_plugin_status( 'spectra-pro/spectra-pro.php' ),
					'templatiq_sites_status'          => $this->get_plugin_status( 'templatiq-sites/templatiq-sites.php' ),
					...
					'suggestion_link'         => 'https://wpastra.com/sites-suggestions/?utm_source=demo-import-panel&utm_campaign=templatiq-sites&utm_medium=suggestions',
					'license_status'          => $license_status,
					'isPro'                   => defined( 'TEMPLATIQ_PRO_SITES_NAME' ) ? true : false,
					'getProURL'               => defined( 'TEMPLATIQ_PRO_SITES_NAME' ) ? esc_url( admin_url( 'plugins.php?bsf-inline-license-form=astra-pro-sites' ) ) : esc_url( 'https://wpastra.com/starter-templates-plans/?utm_source=gutenberg-templates&utm_medium=dashboard&utm_campaign=Starter-Template-Backend' ),
					'templatiq_theme_css'         => isset( $templatiq_theme_css ) ? $templatiq_theme_css : '',
					'site_url'                => site_url(),
					...
					'reset_details_ajax_nonce'             => ( current_user_can( 'manage_options' ) ) ? wp_create_nonce( 'ast-block-templates-reset-business-details' ) : '',
					'business_details'  => get_option( 'ast-templates-business-details', false ),
					...
					'templatiq_customizer_css' => str_replace( 'body', '.st-block-container', $templatiq_customizer_css ),
					...
					'templatiq_static_css_path' => $static_css_path,
				)
			)
		);
	}
	...
	public function get_page_palette_colors() {
		...
		if ( class_exists( 'Templatiq_Global_Palette' ) ) {
			$templatiq_palette_colors = templatiq_get_palette_colors();
			$default_palette_color = $templatiq_palette_colors['palettes'][ $templatiq_palette_colors['currentPalette'] ];
		}
	...
	public function get_all_sites() {
		$total_requests = (int) get_option( 'ast-block-templates-site-requests', 0 );
			...
			for ( $page = 1; $page <= $total_requests; $page++ ) {
				...
				if ( ! empty( $current_page_data ) ) {
					foreach ( $current_page_data as $site_id => $site_data ) {
						...
						// Replace `templatiq-sites-tag` with `tag`.
						if ( isset( $site_data['templatiq-sites-tag'] ) ) {
							$site_data['tag'] = $site_data['templatiq-sites-tag'];
							unset( $site_data['templatiq-sites-tag'] );
						}
						...
						if ( count( $site_data['pages'] ) ) {
							foreach ( $site_data['pages'] as $page_id => $page_data ) {
								...
								// Replace `templatiq-sites-tag` with `tag`.
								if ( isset( $single_page['templatiq-sites-tag'] ) ) {
									$single_page['tag'] = $single_page['templatiq-sites-tag'];
									unset( $single_page['templatiq-sites-tag'] );
								}
	...
	public function get_all_blocks() {
		...
		$total_requests = (int) get_option( 'ast-block-templates-block-requests', 0 );

		for ( $page = 1; $page <= $total_requests; $page++ ) {
			$current_page_data = get_option( 'ast-block-templates-blocks-' . $page, array() );
			if ( ! empty( $current_page_data ) ) {
