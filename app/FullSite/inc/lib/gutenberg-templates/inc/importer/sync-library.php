...
class Sync_Library {
	 ...
	 * @return string
	 */
	public function maybe_get_server_templatiq_customizer_css() {
	...
	public function check_checksum_and_get_blocks_data() {
		...
		$api_url = add_query_arg( $query_args, trailingslashit( AST_BLOCK_TEMPLATES_LIBRARY_URL ) . 'wp-json/templatiq-sites/v2/checksum/' );
	...
	public function set_last_export_checksums() {
		...
		$api_url = add_query_arg( $query_args, trailingslashit( AST_BLOCK_TEMPLATES_LIBRARY_URL ) . 'wp-json/templatiq-sites/v1/get-last-export-checksums/' );
	...
	public function ajax_blocks_requests_count() {
		...
		$this->maybe_get_server_templatiq_customizer_css();
		if ( $total_requests ) {
			wp_send_json_success(
				array(
					'message' => 'Success',
