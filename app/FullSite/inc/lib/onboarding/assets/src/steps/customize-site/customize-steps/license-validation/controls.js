...
const LicenseValidationControls = () => {
	...
	const validateKey = () => {
		if ( licenseKey === '' ) {
			setError( __( 'Please Enter License Key', 'templatiq-sites' ) );
			return;
		}
	...
	if (
		premiumTemplate &&
		'' !== templatiqSitesVars.license_page_builder &&
		...
		return <p>{ __( 'Not Valid License', 'templatiq-sites' ) }</p>;
	}
	...
	return (
		<>
			{ ! whiteLabelEnabled() && (
				<>
					<h4>{ __( 'Already a customer?', 'templatiq-sites' ) }</h4>
