...
if ( ! class_exists( 'Templatiq_Sites_Batch_Processing_Importer' ) ) :
	...
	class Templatiq_Sites_Batch_Processing_Importer {
		...
		public function import_all_categories_and_tags() {
			...
			$request = wp_remote_get( trailingslashit( Templatiq_Sites::get_instance()->get_api_domain() ) . 'wp-content/uploads/all-site-categories.json', $api_args );
		...
		public function import_all_categories() {
			...
			$request = wp_remote_get( trailingslashit( Templatiq_Sites::get_instance()->get_api_domain() ) . 'wp-json/wp/v2/templatiq-sites-site-category/?hide_empty=true&_fields=id,name,slug&per_page=100', $api_args );
			if ( ! is_wp_error( $request ) && 200 === (int) wp_remote_retrieve_response_code( $request ) ) {
				$cats = json_decode( wp_remote_retrieve_body( $request ), true );
		...
		public function import_site_categories() {
			...
			$categories_request = wp_remote_get( trailingslashit( Templatiq_Sites::get_instance()->get_api_domain() ) . 'wp-json/wp/v2/templatiq-sites-site-category/?hide_empty=true&_fields=id,name,slug&per_page=100', $api_args );
		...
		public function import_block_categories() {
			...
			$tags_request = wp_remote_get( trailingslashit( Templatiq_Sites::get_instance()->get_api_domain() ) . 'wp-json/wp/v2/blocks-category/?_fields=id,name,slug&per_page=100&hide_empty=1', $api_args );
		...
		public function import_page_builders() {
			...
			$page_builder_request = wp_remote_get( trailingslashit( Templatiq_Sites::get_instance()->get_api_domain() ) . 'wp-json/wp/v2/astra-site-page-builder/?_fields=id,name,slug&site_url=' . $site_url . '&purchase_key=' . $purchase_key, $api_args );
		...
		public function import_blocks( $page = 1 ) {
			...
			$api_url = add_query_arg( $query_args, trailingslashit( Templatiq_Sites::get_instance()->get_api_domain() ) . 'wp-json/astra-blocks/v1/blocks/' );
		...
		public function import_sites( $page = 1 ) {
			...
			$api_url = add_query_arg( $query_args, trailingslashit( Templatiq_Sites::get_instance()->get_api_domain() ) . 'wp-json/templatiq-sites/v1/sites-and-pages/' );
