...
if ( ! class_exists( 'Templatiq_Sites_Batch_Site_Import' ) ) :
	 ...
	 * @since 2.5.0
	 */
	class Templatiq_Sites_Batch_Site_Import {
		...
		public function __construct() {
			...
			require_once TEMPLATIQ_SITES_DIR . 'inc/classes/batch-import/class-templatiq-sites-batch-site-import-batch.php';

			/** WordPress Plugin Administration API */
			require_once ABSPATH . 'wp-admin/includes/plugin.php';
			require_once ABSPATH . 'wp-admin/includes/update.php';

			self::$site_import_batch = new Templatiq_Sites_Batch_Site_Import_Batch();

			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
			add_action( 'admin_bar_menu', array( $this, 'admin_bar_menu_item' ), 99 );
			add_action( 'wp_ajax_templatiq_sites_check_import_status', array( $this, 'check_import_status' ) );
			add_filter( 'wp_templatiq_sites_site_import_batch_cron_interval', array( $this, 'set_cron_interval' ) );
			...
			if (
				...
				add_filter( 'templatiq_sites_process_sync_batch', '__return_false' );
				add_filter( 'templatiq_sites_process_auto_sync_library', '__return_false' );
			}
		}
		...
		public function check_import_status() {
			...
			// Verify Nonce.
			check_ajax_referer( 'templatiq-sites-import-status', '_ajax_nonce' );
			if ( ! current_user_can( 'edit_posts' ) ) {
				wp_send_json_error();
			}

			$schedule = wp_get_scheduled_event( 'wp_templatiq_sites_site_import_batch_cron' );
			$time     = '';
			if ( $schedule ) {
		...
		public function get_status() {
			return get_option( 'templatiq_sites_site_import_batch_status', array() );
		}
		...
		public function update_status( $args = array() ) {
			...
			update_option( 'templatiq_sites_site_import_batch_status', $args, 'no' );
		}
		...
		public function enqueue_scripts( $hook = '' ) {
			...
			$status = $this->get_status();
			if ( ! empty( $status ) && ( isset( $status['step'] ) && 'complete' !== $status['step'] ) ) {
				wp_enqueue_script( 'templatiq-sites-import-status', TEMPLATIQ_SITES_URI . 'inc/assets/js/import-status.js', array( 'jquery' ), TEMPLATIQ_SITES_VER, true );
				wp_localize_script(
					'templatiq-sites-import-status',
					'TemplatiqSitesImportStatusVars',
					array(
						'ajaxurl'     => esc_url( admin_url( 'admin-ajax.php' ) ),
						'_ajax_nonce' => wp_create_nonce( 'templatiq-sites-import-status' ),
					)
				);
			}
		}
		...
		public function import_flows_and_steps() {
			$demo_data = get_option( 'templatiq_sites_import_data', array() );
			if ( ! empty( $demo_data ) && isset( $demo_data['templatiq-site-cartflows-path'] ) && ! empty( $demo_data['templatiq-site-cartflows-path'] ) ) {
	...
	Templatiq_Sites_Batch_Site_Import::get_instance();
