<?php
/**
 * Templatiq Sites WP CLI
 *
 * 1. Run `wp templatiq-sites list`                     List of all astra sites.
 * 2. Run `wp templatiq-sites import <id>`    Import site.
 *
...
if ( class_exists( 'WP_CLI_Command' ) && ! class_exists( 'Templatiq_Sites_WP_CLI' ) ) :
	...
	/**
	 * WP-Cli commands to manage Templatiq Starter Sites.
	 *
	 * @since 1.4.0
	 */
	class Templatiq_Sites_WP_CLI extends WP_CLI_Command {
		...
		public function batch( $args, $assoc_args ) {
			...
			Templatiq_Sites_Batch_Processing::get_instance()->start_process();
		}

		/**
		 * Generates the list of all Templatiq Sites.
		 *
		 * ## OPTIONS
		 *
		 * [--per-page=<number>]
		...
		public function list_sites( $args, $assoc_args ) {
			...
			$rest_args = array(
				'_fields'  => 'id,title,slug,templatiq-sites-site-category,astra-site-page-builder,astra-site-type,astra-site-url',
				'per_page' => $per_page,
			);
			...
			$list = (array) $this->get_sites( 'templatiq-sites', $rest_args, true, $assoc_args );
			...
			if ( ! empty( $list ) ) {
				...
				WP_CLI::error( __( 'No sites found! Try another query.', 'templatiq-sites' ) );
			}
		}
		 ...
		 *
		 *     # Import demo site.
		 *     $ wp templatiq-sites import 34184 --reset --yes
		 *     Activating Plugins..
		 *     Resetting Posts..
		 *     ..
		 *
		...
		public function import( $args, $assoc_args ) {
			...
			if ( ! $yes ) {
				WP_CLI::confirm( __( 'Are you sure you want to import the site?', 'templatiq-sites' ) );
			}
			...
			if ( is_wp_error( $demo_data ) ) {
				...
				WP_CLI::error( sprintf( __( 'Site Import failed due to error: %s', 'templatiq-sites' ), $demo_data->get_error_message() ) );
			}
			...
			if ( 'free' !== $demo_data['site-type'] && 'upgrade' === $demo_data['license-status'] && ! $license_status ) {
				...
				if ( ! defined( 'TEMPLATIQ_PRO_SITES_NAME' ) ) {
					WP_CLI::line( __( 'This is Premium site. Please activate the "Starter Templates" license!', 'templatiq-sites' ) );
					WP_CLI::line( __( 'Use `wp plugin deactivate templatiq-sites` and then `wp plugin activate astra-pro-sites`', 'templatiq-sites' ) );
				}

				/* translators: %s is the activate plugin license link. */
				WP_CLI::error( __( 'Use CLI command `wp brainstormforce license activate astra-pro-sites {YOUR_LICENSE_KEY}`', 'templatiq-sites' ) );
			}
			...
			if ( isset( $demo_data['required-plugins'] ) ) {
				...
				if ( ! empty( $plugins ) ) {
					...
					if ( ! empty( $plugin_status['required_plugins']['notinstalled'] ) ) {
						WP_CLI::line( __( 'Installing Plugins..', 'templatiq-sites' ) );
			 ...
			 */
			if ( $reset ) {
				WP_CLI::runcommand( 'templatiq-sites reset --yes' );
			}
			...
			if ( isset( $demo_data['astra-site-wxr-path'] ) && ! empty( $demo_data['astra-site-wxr-path'] ) ) {
				WP_CLI::runcommand( 'templatiq-sites import_wxr ' . $demo_data['astra-site-wxr-path'] );
			}
			 ...
			 * Import End.
			 */
			WP_CLI::runcommand( 'templatiq-sites import_end' );

			/* translators: %s is the site URL. */
			WP_CLI::line( sprintf( __( "Site Imported Successfully!\nVisit: %s", 'templatiq-sites' ), $site_url ) );
		}
		...
		public function import_end() {
			Templatiq_Sites_Importer::get_instance()->import_end();
		}
		 ...
		 * ## EXAMPLES
		 *
		 *      $ wp templatiq-sites import_wxr <url>
		 *
		 * @since 1.4.3
		 * @param  array $args       Arguments.
		 * @param  array $assoc_args Associated Arguments.
		...
		public function import_wxr( $args = array(), $assoc_args = array() ) {
			...
			if ( empty( $url ) ) {
				WP_CLI::error( esc_html__( 'Invalid XML URL.', 'templatiq-sites' ) );
			}

			// Download XML file.
			/* translators: %s is the XML file URL. */
			WP_CLI::line( sprintf( esc_html__( 'Downloading %s', 'templatiq-sites' ), $url ) );
			$xml_path = Templatiq_Sites_Helper::download_file( $url );

			if ( $xml_path['success'] && isset( $xml_path['data']['file'] ) ) {
				WP_CLI::line( esc_html__( 'Importing WXR..', 'templatiq-sites' ) );
				Templatiq_WXR_Importer::instance()->sse_import( $xml_path['data']['file'] );
			} else {
				/* translators: %s is error message. */
				WP_CLI::line( printf( esc_html__( 'WXR file Download Failed. Error %s', 'templatiq-sites' ), esc_html( $xml_path['data'] ) ) );
			}
		}
		...
		public function reset( $args = array(), $assoc_args = array() ) {
			...
			$yes = isset( $assoc_args['yes'] ) ? true : false;
			if ( ! $yes ) {
				WP_CLI::confirm( __( 'Are you sure you want to delete imported site data?', 'templatiq-sites' ) );
			}
			...
			if ( isset( $reset_data['reset_posts'] ) && ! empty( $reset_data['reset_posts'] ) ) {
				WP_CLI::line( __( 'Resetting Posts..', 'templatiq-sites' ) );
				foreach ( $reset_data['reset_posts'] as $key => $post_id ) {
					Templatiq_Sites_Importer::get_instance()->delete_imported_posts( $post_id );
				}
			}
			// Delete tracked terms.
			if ( isset( $reset_data['reset_terms'] ) && ! empty( $reset_data['reset_terms'] ) ) {
				WP_CLI::line( __( 'Resetting Terms..', 'templatiq-sites' ) );
				foreach ( $reset_data['reset_terms'] as $key => $post_id ) {
					Templatiq_Sites_Importer::get_instance()->delete_imported_terms( $post_id );
				}
			}
			// Delete tracked WP forms.
			if ( isset( $reset_data['reset_wp_forms'] ) && ! empty( $reset_data['reset_wp_forms'] ) ) {
				WP_CLI::line( __( 'Resting WP Forms...', 'templatiq-sites' ) );
				foreach ( $reset_data['reset_wp_forms'] as $key => $post_id ) {
					Templatiq_Sites_Importer::get_instance()->delete_imported_wp_forms( $post_id );
				}
			}

			// Delete Customizer Data.
			Templatiq_Sites_Importer::get_instance()->reset_customizer_data();

			// Delete Site Options.
			Templatiq_Sites_Importer::get_instance()->reset_site_options();
		 ...
		 *     λ wp templatiq-sites page_builder list
		 *     +----------------+----------------+
		 *     | slug           | name           |
		 *     +----------------+----------------+
		 *     | gutenberg      | Gutenberg      |
		 ...
		 *     λ wp templatiq-sites page_builder set elementor
		 *     "Elementor" is set as default page builder.
		 *
		 *     # Set `Beaver Builder` as default page builder.
		 *     λ wp templatiq-sites page_builder set beaver-builder
		...
		public function page_builder( $args, $assoc_args ) {
			...
			if ( empty( $action ) ) {
				WP_CLI::error( __( 'Please add valid parameter.', 'templatiq-sites' ) );
			}

			$page_builders = Templatiq_Sites_Page::get_instance()->get_page_builders();
			...
			if ( 'list' === $action ) {
				...
				$default_page_builder = isset( $page_builders[ Templatiq_Sites_Page::get_instance()->get_setting( 'page_builder' ) ] ) ? $page_builders[ Templatiq_Sites_Page::get_instance()->get_setting( 'page_builder' ) ]['name'] : '';

				if ( ! empty( $default_page_builder ) ) {
					/* translators: %s is the current page builder name. */
					WP_CLI::line( sprintf( __( 'Default page builder is "%s".', 'templatiq-sites' ), $default_page_builder ) );
				}
			} elseif ( 'set' === $action ) {
				...
				if ( in_array( $page_builder_slug, $page_builder_slugs, true ) ) {
					Templatiq_Sites_Page::get_instance()->save_page_builder_on_submit( $page_builder_slug );
					/* translators: %s is the page builder name. */
					WP_CLI::line( sprintf( __( '"%s" is set as default page builder.', 'templatiq-sites' ), $page_builders[ $page_builder_slug ]['name'] ) );

				} else {
					WP_CLI::error( __( "Invalid page builder slug. \nCheck all page builder slugs with command `wp templatiq-sites page_builder list`", 'templatiq-sites' ) );
				}
			} else {
				WP_CLI::error( __( "Invalid parameter! \nPlease use `list` or `set` parameter.", 'templatiq-sites' ) );
			}
		}
		...
		private function get_sites( $post_slug = '', $args = array(), $force = false, $assoc_args = array() ) {
			...
			// Add page builders.
			$page_builder  = isset( $assoc_args['page-builder'] ) ? $assoc_args['page-builder'] : Templatiq_Sites_Page::get_instance()->get_setting( 'page_builder' );
			...
			if ( empty( $page_builders['data'] ) ) {
				WP_CLI::error( __( 'This page builder plugin is not installed. Please try a different page builder.', 'templatiq-sites' ) );
			}

			// Add type.
			$type     = isset( $assoc_args['type'] ) ? $assoc_args['type'] : '';
			$response = $this->get_term_ids( 'templatiq-sites-type', $type, $args );
			$args     = $response['args'];
			$types    = $response['terms'];
			if ( empty( $types['data'] ) ) {
				WP_CLI::error( __( 'This site type does not exist. Please try a different site type.', 'templatiq-sites' ) );
			}

			// Add categories.
			$category   = isset( $assoc_args['category'] ) ? $assoc_args['category'] : '';
			$response   = $this->get_term_ids( 'templatiq-sites-site-category', $category, $args );
			$args       = $response['args'];
			$categories = $response['terms'];
			if ( empty( $categories['data'] ) ) {
				WP_CLI::error( __( 'This site category does not exist. Please try a different site category.', 'templatiq-sites' ) );
			}

			// Site list.
			$sites = (array) $this->get_posts( 'templatiq-sites', $args, $force );
			...
			if ( $sites['success'] ) {
				foreach ( $sites['data'] as $key => $site ) {
					...
					if ( isset( $site['templatiq-sites-site-category'] ) && ! empty( $categories['data'] ) ) {
						foreach ( $site['templatiq-sites-site-category'] as $category_key => $category_id ) {
			...
			if ( empty( $terms_data ) || $force ) {
				...
				if ( ! is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) === 200 ) {
					...
					if ( ! isset( $request_term_data['code'] ) ) {
						...
						if ( set_transient( 'templatiq-sites-term-' . $term_slug, $new_terms_data, WEEK_IN_SECONDS ) ) {
							return array(
								'success' => $success,
								'data'    => $new_terms_data,
							);
			...
			if ( empty( $all_posts ) || $force ) {
				...
				if ( ! is_wp_error( $response ) || wp_remote_retrieve_response_code( $response ) === 200 ) {
					...
					if ( ! isset( $all_posts['code'] ) ) {
						$success = true;
						set_transient( 'templatiq-sites-post-' . $post_slug, $all_posts, WEEK_IN_SECONDS );
					}
				}
			} else {
				$success = true;
		...
		public function sync( $args = array(), $assoc_args = array() ) {
			Templatiq_Sites_Batch_Processing::get_instance()->process_batch();
		}
	}
	 ...
	 * Add Command
	 */
	WP_CLI::add_command( 'starter-templates', 'Templatiq_Sites_WP_CLI' );
	WP_CLI::add_command( 'templatiq-sites', 'Templatiq_Sites_WP_CLI' );
