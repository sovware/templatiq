...
<script type="text/template" id="tmpl-ast-image-list">
	...
	<# var count = 0; #>
		<# for ( key in data ) { count++; #>
			<# var is_imported = _.includes( templatiqImages.saved_images, data[key]['id'] ); #>
	...
	<div class="ast-image__filter-wrap">
		<ul class="ast-image__filter">
			<li class="ast-image__filter-category">
				<select>
					<# for ( key in templatiqImages.pixabay_category ) { #>
					<option value="{{key}}">{{templatiqImages.pixabay_category[key]}}</option>
					<# } #>
				</select>
			</li>
			<li class="ast-image__filter-orientation">
				<select>
					<# for ( key in templatiqImages.pixabay_orientation ) { #>
					<option value="{{key}}">{{templatiqImages.pixabay_orientation[key]}}</option>
					<# } #>
				</select>
			</li>
			<li class="ast-image__filter-order">
				<select>
					<# for ( key in templatiqImages.pixabay_order ) { #>
					<option value="{{key}}">{{templatiqImages.pixabay_order[key]}}</option>
					<# } #>
				</select>
			</li>
			<li class="ast-image__filter-safesearch">
				<label><input type="checkbox" checked value="1" /><?php esc_html_e( 'SafeSearch', 'templatiq-sites' ); ?></label>
			</li>
		</ul>
	</div>
	<div class="ast-powered-by-pixabay-wrap"><span><?php esc_html_e( 'Powered by', 'templatiq-sites' ); ?></span><img src="<?php echo esc_url( TEMPLATIQ_SITES_URI . 'inc/assets/images/pixabay-logo.png' ); ?>">
	</div>
</script>
...
<script type="text/template" id="tmpl-ast-image-no-result">
	<div class="templatiq-sites-no-sites">
		...
		<p class="description">
			...
			printf( esc_html__( 'Don\'t see a template you would like to import?<br><a target="_blank" href="%1$s">Make a Template Suggestion!</a>', 'templatiq-sites' ), esc_url( 'https://wpastra.com/sites-suggestions/?utm_source=demo-import-panel&utm_campaign=templatiq-sites&utm_medium=suggestions' ) );
			?>
		</p>
	</div>
</script>
...
<script type="text/template" id="tmpl-ast-image-single">
	<# var is_imported = _.includes( templatiqImages.saved_images, data.id.toString() ); #>
...
<script type="text/template" id="tmpl-ast-image-go-back">
	<div class="ast-image__go-back">
		...
		<span class="ast-image__go-back-text"><?php esc_html_e( 'Back to Images', 'templatiq-sites' ); ?></span>
	</div>
</script>
...
<script type="text/template" id="tmpl-ast-image-save">
	...
	<div class="ast-image__save-wrap">
		<button type="button" class="ast-image__save button media-button button-primary button-large media-button-select {{disable_class}}" data-import-status={{is_imported}}>
			<# if ( is_imported ) { #>
				<?php esc_html_e( 'Already Saved', 'templatiq-sites' ); ?>
			<# } else { #>
				<?php esc_html_e( 'Save & Insert', 'templatiq-sites' ); ?>
			<# } #>
		</button>
	</div>
</script>
