...
class AddBuilderSettings {
	...
	public function add_listing_custom_content( $content ) {
		...
		$enable_template = get_directorist_type_option( $term_id, 'enable_add_listing_custom_template' );
		$page_id         = empty( $enable_template ) ? '' : get_directorist_type_option( $term_id, 'custom_add_listing_template' );
		...
		return get_the_content( null, false, $page_id );
	}
