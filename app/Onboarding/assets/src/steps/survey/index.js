...
const Survey = () => {
	...
	if ( notInstalled.length > 0 ) {
		notInstalled.map( ( plugin ) => {
			return allPluginList.push( {
				plugin,
				action: __( 'Install & Activate', 'templatiq-sites' ),
			} );
		} );
	}
	...
	if ( notActivated.length > 0 ) {
		notActivated.map( ( plugin ) => {
			return allPluginList.push( {
				plugin,
				action: __( 'Activate', 'templatiq-sites' ),
			} );
		} );
	}
	...
	const manualPluginInstallation = () => {
		return (
			<form className="install-plugins-form" onSubmit={ recheckPlugins }>
				<h1>{ __( 'Required plugins missing', 'templatiq-sites' ) }</h1>
				<p>
					{ __(
				...
				<h5>{ __( 'Required plugins -', 'templatiq-sites' ) }</h5>
				...
				>
					{ __( 'Start Importing', 'templatiq-sites' ) }
					{ ICONS.arrowRight }
				</button>
			</form>
		);
	...
	const compatibilities = templatiqSitesVars.compatibilities;
	...
	if (
		...
		const tooltipString =
			templatiqSitesVars.compatibilities_data[ 'update-available' ];
	...
	const handleSurveyFormSubmit = ( e ) => {
		...
		if ( analytics !== 'yes' ) {
			...
			optinAnswer.append( 'action', 'templatiq-sites-update-analytics' );
			optinAnswer.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
		...
		if ( templatiqSitesVars.subscribed === 'yes' ) {
			dispatch( {
				type: 'set',
		...
		const content = new FormData();
		content.append( 'action', 'templatiq-sites-update-subscription' );
		content.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
	...
	const surveyForm = () => {
		return (
			<form className="survey-form" onSubmit={ handleSurveyFormSubmit }>
				<h1>{ __( 'Okay, just one last step…', 'templatiq-sites' ) }</h1>
				{ templatiqSitesVars.subscribed !== 'yes' && (
				...
				<button
					...
					className="submit-survey-btn button-text d-flex-center-align"
				>
					{ __( 'Submit & Build My Website', 'templatiq-sites' ) }
					{ ICONS.arrowRight }
				</button>
			</form>
		);
		...
		return (
			...
			>
				...
				<button
					...
					className="submit-survey-btn button-text d-flex-center-align"
				>
					{ __( 'Skip & Start Importing', 'templatiq-sites' ) }
					{ ICONS.arrowRight }
				</button>
			</form>
		);
	...
	const hardRequirement = () => {
		return (
			<div className="requirement-check-wrap">
				<h1>{ __( "We're Almost There!", 'templatiq-sites' ) }</h1>
		...
		return (
			<div className="requirement-check-wrap">
				...
				<ul className="requirement-check-list">
					<li>
						<div className="requirement-list-item">
							{ __( 'Read Permissions:', 'templatiq-sites' ) }
							<span
								className={ `dashicons ${
									isReadable
										? 'dashicons-yes'
					...
					<li>
						<div className="requirement-list-item">
							{ __( 'Write Permissions:', 'templatiq-sites' ) }
							<span
								className={ `dashicons ${
									isWritable
										? 'dashicons-yes'
