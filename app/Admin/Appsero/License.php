...
namespace Templatiq\Admin\Appsero;
...
class License {
    ...
    protected function send_request( $license_key, $route ) {
        ...
        if ( empty( $response ) || isset( $response['exception'] ) ) {
            return [
                'success' => false,
                'error'   => $this->client->_trans( 'Unknown error occurred, Please try again.' ),
            ];
        }
        ...
        <div class="wrap appsero-license-settings-wrapper">
            ...
            <div class="appsero-license-settings appsero-license-section">
                ...
                <div class="appsero-license-details">
                    <p>
                        <?php printf( $this->client->_trans( 'Activate <strong>%s</strong> by your license key to get professional support and automatic update from your WordPress dashboard.' ), $this->client->name ); ?>
                    </p>
                        ...
                        <div class="license-input-fields">
                            <div class="license-input-key">
                                ...
                                <input type="text" value="<?php echo $this->get_input_license_value( $action, $license ); ?>"
                                    placeholder="<?php echo esc_attr( $this->client->_trans( 'Enter your license key to activate' ) ); ?>" name="license_key"
                                    <?php echo ( 'deactive' === $action ) ? 'readonly="readonly"' : ''; ?>
                                />
                            </div>
                            <button type="submit" name="submit" class="<?php echo 'deactive' === $action ? 'deactive-button' : ''; ?>">
                                <?php echo $action === 'active' ? $this->client->_trans( 'Activate License' ) : $this->client->_trans( 'Deactivate License' ); ?>
                            </button>
                        </div>
                    </form>
    ...
    public function license_form_submit( $form_data = array() ) {
        ...
        if ( ! wp_verify_nonce( sanitize_key( wp_unslash( $form_data['_nonce'] ) ), $this->client->name ) ) {
            $this->error = $this->client->_trans( 'Nonce vefification failed.' );
            ...
            <div class="single-license-info">
                ...
                <?php } else { ?>
                    <p class="<?php echo $license['remaining'] ? '' : 'occupied'; ?>">
                        <?php printf( $this->client->_trans( '%1$d out of %2$d' ), $license['remaining'], $license['activation_limit'] ); ?>
                    </p>
                <?php } ?>
            </div>
            <div class="single-license-info">
                ...
                if ( false !== $license['expiry_days'] ) {
                    ...
                    echo '<p>' . $this->client->_trans( 'Never' ) . '</p>';
                }
                ?>
            </div>
        </div>
    ...
    private function show_license_page_card_header( $license ) {
        ...
        <div class="appsero-license-title">
            ...
            <span><?php echo $this->client->_trans( 'Activate License' ); ?></span>
            ...
            <form method="post" class="appsero-license-right-form" novalidate="novalidate" spellcheck="false">
                ...
                <button type="submit" name="submit" class="appsero-license-refresh-button">
                    ...
                    <?php echo $this->client->_trans( 'Refresh License' ); ?>
                </button>
            </form>
            <?php } ?>
    ...
    private function active_client_license( $license_key ) {
        if ( empty( $license_key ) ) {
            $this->error = $this->client->_trans( 'The license key field is required.' );
        ...
        if ( ! $response['success'] ) {
            $this->error = $response['error'] ? $response['error'] : $this->client->_trans( 'Unknown error occurred.' );
        ...
        $this->success = $this->client->_trans( 'License activated successfully.' );
    }
    ...
    private function deactive_client_license() {
        ...
        if ( empty( $license['key'] ) ) {
            $this->error = $this->client->_trans( 'License key not found.' );
        ...
        $this->success = $this->client->_trans( 'License deactivated successfully.' );
    }
    ...
    private function refresh_client_license() {
        ...
        $this->success = $this->client->_trans( 'License refreshed successfully.' );
    }
