...
if ( ! class_exists( 'Templatiq_Sites_Batch_Processing_Gutenberg' ) ) :

	/**
	 * Templatiq Sites Batch Processing Brizy
	 *
	 * @since 1.2.14
	 */
	class Templatiq_Sites_Batch_Processing_Gutenberg {
		...
		public function import() {
			...
			Templatiq_Sites_Importer_Log::add( '---- Processing WordPress Posts / Pages - for "Gutenberg" ----' );

			$post_types = apply_filters( 'templatiq_sites_gutenberg_batch_process_post_types', array( 'page', 'wp_block', 'wp_template', 'wp_navigation', 'wp_template_part', 'wp_global_styles', 'sc_form' ) );
			if ( defined( 'WP_CLI' ) ) {
		...
		public function import_single_post( $post_id = 0 ) {
			...
			Templatiq_Sites_Importer_Log::add( '---- Processing WordPress Page - for Gutenberg ---- "' . $post_id . '"' );
		...
		public function get_content( $content = '' ) {
			...
			foreach ( $all_links as $key => $link ) {
				if ( templatiq_sites_is_valid_image( $link ) ) {
	 ...
	 */
	Templatiq_Sites_Batch_Processing_Gutenberg::get_instance();
