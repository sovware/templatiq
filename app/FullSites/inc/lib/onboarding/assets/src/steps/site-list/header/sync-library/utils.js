...
export const SyncStart = async () => {
	...
	try {
		const formData = new FormData();
		formData.append( 'action', 'templatiq-sites-update-library' );
...
export const SyncLibraryComplete = async () => {
	...
	try {
		const formData = new FormData();
		formData.append( 'action', 'templatiq-sites-update-library-complete' );
		formData.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
		const response = await fetch( ajaxurl, {
			method: 'post',
			body: formData,
...
export const SyncBlockCategories = async () => {
	...
	try {
		const formData = new FormData();
		formData.append( 'action', 'templatiq-sites-import-page-builders' );
		formData.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
		const response = await fetch( ajaxurl, {
			method: 'post',
			body: formData,
...
export const SyncBlocks = async () => {
	...
	try {
		const formData = new FormData();
		formData.append( 'action', 'templatiq-sites-get-blocks-request-count' );
		formData.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
		...
		if ( totalBlocks ) {
			...
			for ( let i = 1; i <= totalBlocks; i++ ) {
				...
				_formData.append( 'action', 'templatiq-sites-import-blocks' );
				_formData.append( 'page_no', i );
				_formData.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
...
export const SyncAllCategories = async () => {
	...
	try {
		const formData = new FormData();
		formData.append( 'action', 'templatiq-sites-import-all-categories' );
		formData.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
		const response = await fetch( ajaxurl, {
			method: 'post',
			body: formData,
...
export const SyncAllCategoriesAndTags = async () => {
	...
	try {
		...
		formData.append(
			'action',
			'templatiq-sites-import-all-categories-and-tags'
		);
		formData.append( '_ajax_nonce', templatiqSitesVars._ajax_nonce );
		const response = await fetch( ajaxurl, {
			method: 'post',
	...
	try {
		...
		if ( totalRequest ) {
			...
			for ( let i = 1; i <= totalRequest; i++ ) {
				const formData = new FormData();
				formData.append( 'action', 'templatiq-sites-import-sites' );
...
export const SyncAndGetAllCategories = async () => {
	try {
		const response = await fetch( ajaxurl, {
			method: 'post',
			body: getFormData( 'action', 'templatiq-sites-get-all-categories' ),
		} );
...
export const SyncAndGetAllCategoriesAndTags = async () => {
	try {
		const response = await fetch( ajaxurl, {
			...
			body: getFormData(
				'action',
				'templatiq-sites-get-all-categories-and-tags'
			),
		} );
		const data = await response.json();
